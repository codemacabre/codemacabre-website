...
I love Linux, and I'm especially fond of having a minimal system with no bloatware. I've distro-hopped a number of times and alternated between Debian and Arch a few times before finally trying [Debian Testing](https://wiki.debian.org/DebianTesting). This rolling release version of Debian is exactly what I was looking for and I've been very happy with it for months.
...
Log in as a root user, then run the edit the `sources.list` file with,
...
Press `Ctrl` + `x` to exit, and save the file. Update the system with,
...
Reboot, and we will be greeted by a LightDM login screen which will open up a working minimal Xfce desktop environment.
...
<Image src={DesktopImg} alt="A minimal Xfce desktop environment with a narrow dark grey panel along the bottom. The background wallpaper features a still from the series 'Scavengers Reign'." class="greyscale" />
...
We can add a small amount of CSS to adjust the panel spacing. These values work for me, but we can modify them as appropriate to our taste.

Create a file called `gtk.css` in the `~/.config/gtk-3.0/` directory, and add the following CSS:
