...
A backlink displays other notes that reference the current note and is a staple of most personal knowledge systems (PKMs). Howeveer, it's not a feature that [Astro](https://astro.build) supports natively, so a bit of customisation is needed.
...
```
 ...
 Now we have this backlinks object, we could put this into the code fence at the top of a component, then iterate over the `backlinks` object to populate a list of backlinks for each note.
